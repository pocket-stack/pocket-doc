...
const act = (globalThis as any).__docAct as (run: () => void) => void;
...
act(() => { s.jump(codeRow * 20 / (s.doc()!.rows * 20 - 194), "document"); }); await frames(100); await shot("syntax-code");
...
act(() => { s.jump(0, "document"); }); await frames(30);

act(() => { s.jump(0.4, "library"); }); await frames(40);
...
s.libraryScroll.stop(); act(() => { s.jump(0, "library"); }); await frames(40);
...
withholdTiles = true;
act(() => { s.jump(tableRow * 20 / ((s.doc()!.rows * 20) - 194), "document"); }); await frames(20);
...
act(() => { s.jump(0, "document"); }); await frames(100);
...
act(() => { s.toggleSelect(); }); act(() => { s.moveCaret(s.draft()!.text.length); });
await command("document", "paste");
check(s.draft()!.text.endsWith(selection), "R menu pastes the selected text locally");
await frames(40); act(() => { s.moveCaret(-s.caret()); }); act(() => { s.moveCaret(1); }); act(() => { s.moveCaret(-1); });
...
act(() => { s.moveCaret(1); }); check(s.caretVisible(), "Moving the caret restarts its visible phase immediately");
...
s.editorScroll.stop(); act(() => { s.moveCaret(-s.caret()); }); await frames(1); await shot("edit");
if (!s.selecting()) act(() => { s.toggleSelect(); });
...
act(() => { s.key("u"); }); const editBeforeUndo = s.draft()!.text;
...
connected = 3; await frames(50); act(() => { s.setSelected(512); }); s.libraryScroll.scrollTo(512 * 24, { immediate: true }); act(() => { s.setFocus("library"); }); await frames(50); act(() => { s.activate(); }); await frames(100); act(() => { s.edit(); }); await frames(40);
...
act(() => { s.moveCaret(phrase + 7 - s.caret()); }); await frames(1);
...
act(() => { s.moveCaret(-s.caret()); }); const stickAt = s.caret(), stickText = s.draft()!.text;
...
act(() => { s.perform("discard"); }); await frames(16); act(() => { s.discard(); }); await frames(40);
act(() => { s.edit(); }); await frames(35);
act(() => { s.moveCaret(-s.caret()); }); act(() => { s.key("local-start "); });
const beforeFar = s.draft()!.text;
act(() => { s.seekEditor(1500); }); await frames(1); act(() => { s.key("during-flight "); }); await frames(70);
...
act(() => { s.seekEditor(0); }); await frames(50);
...
loseSeekReply = true; act(() => { s.key("retry-once "); }); act(() => { s.seekEditor(2200); }); await frames(1);
...
act(() => { s.moveCaret(-s.caret()); });
const fill = "z".repeat(768 - s.draft()!.text.length); act(() => { s.key(fill); }); act(() => { s.key("q"); });
...
act(() => { s.seekEditor(1700); }); await frames(50); act(() => { s.moveCaret(s.draft()!.text.length - s.caret()); }); await frames(50);
// Find the actual last loaded row after prefetch settles, then cross it once.
act(() => { s.moveCaret(s.draft()!.text.length - s.caret()); }); const lastLoaded = s.caretRow(); act(() => { s.dragCaret(0, 1); }); await frames(40);
check(s.caretRow() === lastLoaded + 1, "One downward caret movement crosses a source window boundary exactly once");
act(() => { s.seekEditor(1700); }); await frames(50); act(() => { s.moveCaret(-s.caret()); });
const deleteAt = s.draft()!.start; act(() => { s.key("DEL"); }); await frames(60);
...
act(() => { s.seekEditor(s.editorTotal() - 8); }); await frames(60);
check(s.draft()!.end === s.draft()!.chars, "Editor reaches the end of a 100 KB document");
act(() => { s.moveCaret(s.draft()!.text.length - s.caret()); }); act(() => { s.key("end-marker"); });
await shot("edit-end"); act(() => { s.save(); }); await frames(100);
...
await shot("new-document"); await tap(55, 44); act(() => { s.key("orkbench"); }); await tap(280, 13); await frames(60);
...
act(() => { s.key("unsaved"); }); act(() => { s.seekEditor(0); }); await frames(30);
act(() => { s.perform("discard"); }); await frames(16); await shot("discard-lowered"); act(() => { s.discard(); }); await frames(40);
...
act(() => { s.edit(); }); await frames(35); act(() => { s.moveCaret(s.draft()!.text.length - s.caret()); }); act(() => { s.key("Two lines"); }); act(() => { s.save(); }); await frames(100);
...
act(() => { s.edit(); }); await frames(35); act(() => { s.key("retained "); }); act(() => { s.perform("read"); });
...
s.libraryScroll.scrollTo(0, { immediate: true }); act(() => { s.setSelected(0); }); await frames(50);
...
act(() => { s.perform("search"); }); act(() => { s.key("workbench"); }); act(() => { s.search(); }); await frames(60);
...
act(() => { s.perform("search"); }); for (let i = 0; i < 9; i++) act(() => { s.key("DEL"); }); act(() => { s.search(); }); await frames(100);
